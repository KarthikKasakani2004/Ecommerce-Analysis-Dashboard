# E-Commerce Sales & Business Analytics Dashboard

An interactive **E-Commerce Sales Analytics Dashboard built in Microsoft Excel** to analyze sales performance, customer behavior, product performance, payments, reviews, and delivery operations.

The project demonstrates practical **data cleaning, data analysis, KPI development, PivotTables, PivotCharts, slicers, timelines, and dashboard design** skills relevant to entry-level Data Analyst roles.

## 📊 Dashboard Preview

<p align="center">
  <img src="./screenshots/Ecommerce Analysis Dashboard.png" alt="E-Commerce Analytics Dashboard">
</p>

## 🎯 Project Objective

The objective of this project is to transform raw e-commerce data into an interactive business dashboard that helps identify:

* Sales trends over time
* High-performing product categories
* Customer and regional performance
* Payment behavior
* Order and delivery patterns
* Customer ratings and reviews
* Returns and operational issues
* Key business performance indicators

## 🛠️ Tools & Skills

**Tool**

* Microsoft Excel

**Data Analysis**

* Data Cleaning
* Exploratory Data Analysis
* KPI Analysis
* Trend Analysis
* Business Insights

**Excel Features**

* PivotTables
* PivotCharts
* Slicers
* Timeline
* Calculated Columns
* Conditional Formatting
* Dynamic Dashboard Elements
* Data Aggregation
* Interactive Filtering

## 📁 Workbook Structure

| Sheet         | Description                                                                 |
| ------------- | --------------------------------------------------------------------------- |
| **Dashboard** | Interactive dashboard containing KPIs, charts and filters                   |
| **Pivot**     | PivotTable-based calculations used to generate dashboard insights           |
| **Orders**    | Order-level sales, discount, shipping and order-status information          |
| **Customers** | Customer demographics and acquisition information                           |
| **Products**  | Product, category, brand and pricing information                            |
| **Payments**  | Payment methods, payment status, fees, net sales and fraud-risk information |
| **Reviews**   | Customer ratings, reviews, delivery/product quality and return information  |

## 📈 Key Dashboard Components

### KPI Analysis

The dashboard includes key performance indicators to provide a quick overview of business performance.

* Total Sales
* Year-wise Sales Performance
* Order-related KPIs
* Customer and operational metrics

### Sales Analysis

The dashboard analyzes:

* Monthly sales trends
* Year-wise sales performance
* Category-level sales
* Product performance
* Regional sales performance

### Customer Analysis

Customer-related analysis includes:

* Customer segments
* Customer acquisition channels
* Regional distribution
* Customer purchasing behavior

### Payment Analysis

Payment data is analyzed based on:

* Payment method
* Payment status
* Transaction fees
* Net sales
* Fraud-risk indicators

### Delivery & Review Analysis

The dashboard also provides insights into:

* Delivery performance
* Shipping delays
* Customer ratings
* Product quality
* Delivery quality
* Returns

## 🔎 Business Questions

This dashboard can help answer questions such as:

1. How are sales changing over time?
2. Which year generated higher sales?
3. Which product categories contribute the most to sales?
4. Which regions generate the highest sales?
5. Which customer segments contribute most to business performance?
6. Which payment methods are most frequently used?
7. What payment issues or risks exist?
8. Where are delivery delays occurring?
9. How do customer ratings vary across products or categories?
10. What patterns can be observed in returns and customer reviews?

## 💡 Key Skills Demonstrated

This project demonstrates my ability to:

* Work with structured business datasets
* Clean and organize data for analysis
* Create calculated columns and metrics
* Build PivotTables and PivotCharts
* Design interactive dashboards
* Create KPI cards
* Use slicers and timelines for dynamic filtering
* Analyze sales and operational performance
* Translate raw data into business insights
* Present analytical results in a clear visual format

## 📂 Project Files

```text
Ecommerce-Analysis-Dashboard/
│
├── Ecommerce_Analysis_Dashboard.xlsx
├── README.md
├── .gitignore
│
└── screenshots/
    └── dashboard.png
```

## 🚀 How to Use

1. Download `Ecommerce_Analysis_Dashboard.xlsx`.
2. Open the workbook using Microsoft Excel.
3. Navigate to the **Dashboard** sheet.
4. Use the slicers and timeline to filter the analysis.
5. Select different years, categories, regions, customer segments and other available filters.
6. Review the KPI cards and charts to identify business trends and patterns.

## 📌 Project Highlights

* Interactive Excel dashboard
* Multiple related e-commerce datasets
* KPI-driven business analysis
* Year and date-based filtering
* Sales trend analysis
* Product and category analysis
* Customer segmentation analysis
* Payment analysis
* Delivery and review analysis
* PivotTable and PivotChart implementation
* Business-focused data visualization

## 👨‍💻 Author

**Karthik Kasakani**

B.Tech — Computer Science & Engineering (AI & ML)

Aspiring Data Analyst

**Skills:** Excel | SQL | Python | Data Analysis | Data Visualization

[LinkedIn](https://www.linkedin.com/in/karthik-kasakani)

[GitHub](https://github.com/KarthikKasakani2004)
